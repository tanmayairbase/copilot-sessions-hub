...
const collectClaudeToolResults = (
  ...
  for (const line of lines) {
    ...
    for (const block of toClaudeBlocks(line.message?.content)) {
      ...
      const resultText = firstString(block.content)
  ...
  for (const block of toClaudeBlocks(content)) {
    ...
    for (const rawQuestion of rawQuestions) {
      ...
      questions.push({
        ...
        answer: answersByQuestion.get(questionText) ?? resultText ?? ''
      })
    }
  }
...
const aggregateClaudeTokenUsage = (
  ...
  const totals = byModel.reduce<SessionTokenUsageTotals>(
    (acc, entry) => ({
      ...
      cacheWriteTokens: acc.cacheWriteTokens + entry.cacheWriteTokens,
      cacheWrite1hTokens:
        (acc.cacheWrite1hTokens ?? 0) + (entry.cacheWrite1hTokens ?? 0),
      outputTokens: acc.outputTokens + entry.outputTokens,
      reasoningTokens: acc.reasoningTokens + entry.reasoningTokens
    }),
    { ...ZERO_TOTALS, cacheWrite1hTokens: 0 }
  )

  return { source: 'claude-messages', byModel, totals }
}

const parseClaudeCodeSessionLog = (
  raw: string,
  context: ParseContext
): ParsedSession[] => {
  ...
  const aiTitle = firstString(
    lines.find(line => line.type === 'ai-title')?.aiTitle
  )
  const titleSeed =
    aiTitle ??
    messages.find(message => message.role === 'user')?.content ??
...
export const parseSessionArtifacts = (
  ...
  if (
    context.filePath.includes('/.claude/projects/') ||
    context.filePath.includes('\\.claude\\projects\\')
  ) {
    return parseClaudeCodeSessionLog(raw, context)
  }
